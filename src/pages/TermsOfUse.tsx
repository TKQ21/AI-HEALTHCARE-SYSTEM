...
import { Activity, Moon, Sun, FileText } from "lucide-react";
  ...
  const sections = [
    {
      ...
      color: "hsl(0,100%,65%)",
      body: "This platform provides clinical decision support only and does not provide medical diagnosis or treatment.",
    },
    {
      title: "User Responsibility",
      color: "hsl(55,100%,60%)",
      body: "Users must seek professional medical advice for any health concerns. In case of emergency, contact local emergency services immediately.",
    },
    {
      title: "Limitation of Liability",
      color: "hsl(320,100%,60%)",
      body: "AI Healthcare System is not responsible for decisions made based on system outputs.",
    },
    {
      title: "Platform Modifications",
      color: "hsl(145,100%,55%)",
      body: "We reserve the right to update or modify the platform and these terms at any time without prior notice.",
    },
    {
      title: "Acceptance of Terms",
      color: "hsl(197,100%,60%)",
      body: "Continued use of this platform indicates acceptance of these terms.",
    },
  ];
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground overflow-x-hidden transition-colors duration-500">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <div className="absolute rounded-full blur-3xl" style={{ width: 500, height: 500, right: "-10%", top: "-5%", background: dark ? "radial-gradient(circle, hsl(145 100% 50% / 0.12), transparent 70%)" : "radial-gradient(circle, hsl(145 100% 50% / 0.05), transparent 70%)" }} />
        <div className="absolute rounded-full blur-3xl" style={{ width: 400, height: 400, left: "-5%", bottom: "20%", background: dark ? "radial-gradient(circle, hsl(55 100% 55% / 0.10), transparent 70%)" : "radial-gradient(circle, hsl(55 100% 55% / 0.04), transparent 70%)" }} />
      </div>
      ...
      <main className="relative z-10 max-w-3xl mx-auto px-6 py-16">
        ...
        <div className="mb-10 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border text-xs font-semibold tracking-widest uppercase mb-5"
            style={{ borderColor: "hsl(145,100%,55%)", color: dark ? "hsl(145,100%,65%)" : "hsl(145,60%,30%)", boxShadow: dark ? "0 0 12px hsl(145 100% 55% / 0.4)" : "none" }}>
            <FileText className="w-3.5 h-3.5" />
            Legal Document
          </div>
          <h1 className="text-4xl md:text-5xl font-black mb-3"
            style={dark ? {
              background: "linear-gradient(135deg, hsl(145,100%,65%), hsl(55,100%,60%))",
        ...
        <p className="text-base leading-relaxed text-muted-foreground mb-10 text-center">
          By accessing AI Healthcare System, you agree to the following terms:
        </p>
        ...
        <div className="flex flex-col gap-6">
          {sections.map((s, i) => (
            <div
              key={s.title}
              className="rounded-2xl border p-6 backdrop-blur-sm flex gap-4 items-start"
              style={{
            ...
            >
              <span className="text-3xl font-black opacity-20 leading-none mt-0.5 flex-shrink-0" style={{ color: s.color }}>
                {String(i + 1).padStart(2, "0")}
              </span>
              <div>
                <h2 className="text-lg font-bold mb-2" style={{ color: dark ? s.color : "hsl(220,80%,20%)", textShadow: dark ? `0 0 8px ${s.color}60` : "none" }}>
                  {s.title}
                </h2>
